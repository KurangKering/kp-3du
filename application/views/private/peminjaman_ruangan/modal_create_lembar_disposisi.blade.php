    ...
    <div class="card-body">
      <div class="form-group row">
        ...
        <div class="col-md-9 col-form-label">
          @foreach (hIsDisposisiRoles() as $index => $role)

          <div class="form-check">
            <input class="form-check-input" id="radio{{ $index }}" type="radio" value="{{ $index }}" name="radios">
            <label class="form-check-label" for="radio{{ $index }}">{{ $role }}</label>
          </div>
          @endforeach
...
<script>
  ...
  {
    var url = SITE_URL + 'private/lembar_disposisi/' + uri;
